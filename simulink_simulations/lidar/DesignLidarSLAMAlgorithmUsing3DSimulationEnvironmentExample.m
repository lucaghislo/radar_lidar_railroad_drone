...
% <docid:driving_ref#mw_1d4e969b-1379-4cc6-a548-59a8c5d59749 |US City
% Block|> scene using the
% <docid:driving_ref#mw_96a521fd-316f-497b-bc01-b2c5f4083563 |Simulation 3D
% Scene Configuration|> block. The model places a vehicle on the scene
% using the <docid:driving_ref#mw_32cd8e72-2d69-4c3e-98b0-5b918db383a4
% |Simulation 3D Vehicle with Ground Following|> block. A lidar sensor is
% attached to the vehicle using the
...
% <docid:driving_ref#mw_f4415107-fd18-4e5c-a55a-4065e669f910 |insSensor|>
% object, and saved in a MAT file.
%
...
% the workspace using a <docid:simulink_ref#f8-894542 |To Workspace|>
% block. The *Visualize Point Cloud* MATLAB Function block uses a
...
% Once the model has completed simulation, the |simOut| variable holds a
% structure with variables written to the workspace. The
% |helperGetPointCloud| function extracts the sensor data into an array of
% <docid:vision_ref#buphqma-1 |pointCloud|> objects. The
% <docid:vision_ref#buphqma-1 |pointCloud|> object is the fundamental data
% structure used to hold lidar data and perform point cloud processing in
% MATLAB. Additionally, INS data is loaded from a MAT file, which will
% later be used to develop the perception algorithm. The INS data was
% obtained using the
% <docid:driving_ref#mw_f4415107-fd18-4e5c-a55a-4065e669f910 |insSensor|>
% object. The INS data has been processed to contain [x, y, theta] poses in
% world coordinates.
...
% 
% * <docid:vision_ref#mw_c6968c3b-08dd-4aaa-94e7-b6c0e0185f8d |pcviewset|>:
% Manages data associated with point cloud odometry like point clouds,
% poses and connections.
% * <docid:vision_ref#mw_79434e5c-58ee-4c82-95f0-c9becef45429
% |scanContextDescriptor|>: Extracts scan context descriptors from each
% incoming point cloud. Scan context is a 2-D global feature descriptor that
% is used for loop closure detection.
% * <docid:vision_ref#mw_058b7ce1-446b-4bc2-8eb1-5721a210c2dd
% |scanContextLoopDetector|>: Manages scan context descriptors and detects
% loop closures. It uses
% <docid:vision_ref#mw_1029b0b4-c1d4-40a7-bde9-fe6a9f7eb61d
% |scanContextDistance|> to compute the distance between scan context
% descriptors and select the closest feature matches.
%
% Then, the example uses point cloud registration to accept or reject loop
...
% |createPoseGraph|> to create a pose graph and
% <docid:nav_ref#mw_ead0d07b-dffc-4389-93ef-8fa1432c60cb
% |optimizePoseGraph|> to optimize the pose graph.
%
...
%%%
% *helperGetPointCloud* Extract an array of <docid:vision_ref#bueuyq5-1
% |pointCloud|> objects.
